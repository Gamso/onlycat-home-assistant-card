 ...
 */function mt(t){return ut({...t,state:!0,attribute:!1})}const gt={en:{card:{name_default:"Cat Flap",config_required:"Please configure the OnlyCat card.",locked:"Locked",unlocked:"Unlocked",connected:"Connected",offline:"Offline",unavailable:"Unavailable",policy:"Policy",no_recent_activity:"No recent activity"},actions:{unlock:"Unlock",unlock_title:"Unlock now",restart:"Restart",restart_title:"Restart the cat flap",cancel:"Cancel"},camera:{title:"Last activity",stream_unavailable:"Stream unavailable."},history:{title:"Activity history",loading:"Loading…",error:"Unable to load history.",passage_detected:"Passage detected",prey_detected:"Prey detected",human_detected:"Human detected",row_flap:"Passage",row_prey:"Prey",row_human:"Human",chart_now:"now"},time:{just_now:"just now",minutes_ago:"{n} min ago",hours_ago:"{h}h ago",hours_minutes_ago:"{h}h{m} ago"},confirm_restart:{title:"Confirm restart",question:"Are you sure you want to restart the cat flap?",note:"The cat flap will be temporarily offline during the restart."},editor:{card_name:"Card name",device:"OnlyCat device",device_hint:"Select the connectivity sensor of your OnlyCat device",show_title:"Show title"}},fr:{card:{name_default:"Chatière",config_required:"Veuillez configurer la carte OnlyCat.",locked:"Verrouillé",unlocked:"Ouvert",connected:"Connecté",offline:"Hors ligne",unavailable:"Indisponible",policy:"Politique",no_recent_activity:"Aucune activité récente"},actions:{unlock:"Déverrouiller",unlock_title:"Déverrouiller maintenant",restart:"Redémarrer",restart_title:"Redémarrer la chatière",cancel:"Annuler"},camera:{title:"Dernière activité",stream_unavailable:"Flux indisponible."},history:{title:"Historique des activités",loading:"Chargement…",error:"Impossible de charger l'historique.",passage_detected:"Passage détecté",prey_detected:"Proie détectée",human_detected:"Humain détecté",row_flap:"Passage",row_prey:"Proie",row_human:"Humain",chart_now:"maintenant"},time:{just_now:"à l'instant",minutes_ago:"il y a {n} min",hours_ago:"il y a {h}h",hours_minutes_ago:"il y a {h}h{m}"},confirm_restart:{title:"Confirmer le redémarrage",question:"Êtes-vous sûr de vouloir redémarrer la chatière ?",note:"La chatière sera temporairement hors ligne pendant le redémarrage."},editor:{card_name:"Nom de la carte",device:"Appareil OnlyCat",device_hint:"Sélectionnez le capteur de connectivité de votre appareil OnlyCat",show_title:"Afficher le titre"}}};function ft(t,e){const i=e.indexOf("."),o=e.slice(0,i),s=e.slice(i+1),r=t[o];return"object"==typeof r?r[s]:void 0}function vt(t,e){return ft(gt[function(t){return(t?.locale?.language??t?.language??"en").toLowerCase().startsWith("fr")?"fr":"en"}(t)],e)??ft(gt.en,e)??e}function yt(t,e,i){let o=vt(t,e);for(const[t,e]of Object.entries(i))o=o.replace(`{${t}}`,String(e));return o}function _t(t){const e=t.match(/^binary_sensor\.(.+)_connectivity$/);return e?e[1]:t}class bt extends lt{_t(t){return vt(this.hass,t)}setConfig(t){this._config=t}_connectivityEntities(){return this.hass?.states?Object.keys(this.hass.states).filter(t=>/^binary_sensor\..+_connectivity$/.test(t)):[]}_valueChanged(t){const e=t.target,i=e.getAttribute("data-key");if(!i)return;let o=e.value;"checkbox"===e.type?o=e.checked:"number"===e.type&&(o=Number(e.value)),this._config={...this._config,[i]:o},this._fire()}_fire(){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config}}))}get _selectedConnectivityEntity(){return this._config?.device_id?`binary_sensor.${this._config.device_id}_connectivity`:""}render(){if(!this._config)return W``;const t=this._connectivityEntities(),e=this._selectedConnectivityEntity;return W`
      <div class="editor">
        ...
        <div class="field">
          <label>
            ${vt(this.hass,"editor.device")} <span class="required">*</span>
          </label>
          ${t.length>0?W`
                <select
                  class="entity-select"
                  .value=${e}
                  @change=${t=>{const e=t.target.value;e&&(this._config={...this._config,device_id:_t(e)},this._fire())}}
                >
                  <option value="" ?selected=${!e}>
                    — ${vt(this.hass,"editor.device")} —
                  </option>
                  ${t.map(t=>W`<option
                        value="${t}"
                        ?selected=${t===e}
              ...
              `:W`
                ...
                />
              `}
          <span class="hint">${vt(this.hass,"editor.device_hint")}</span>
        ...
        <div class="field field--checkbox">
          <label>
            ...
            />
            ${vt(this.hass,"editor.show_title")}
          </label>
        </div>
      </div>
    `}}bt.styles=n`
  ...
  `,t([ut({attribute:!1})],bt.prototype,"hass",void 0),t([mt()],bt.prototype,"_config",void 0),customElements.define("onlycat-home-assistant-card-editor",bt);class $t extends lt{constructor(){super(...arguments),this._showModal=!1}_entity(){return this.hass?.states?.[this.entityId]}_getSnapshotUrl(){const t=this._entity()?.attributes?.entity_picture;return t??null}_getVideoUrl(){return this._entity()?.attributes?.video_url??null}_relativeTime(t){if(!t)return"";const e=new Date(t);if(isNaN(e.getTime()))return"";const i=Math.round((Date.now()-e.getTime())/6e4);if(i<1)return vt(this.hass,"time.just_now");if(i<60)return yt(this.hass,"time.minutes_ago",{n:i});const o=Math.floor(i/60),s=i%60;return 0===s?yt(this.hass,"time.hours_ago",{h:o}):yt(this.hass,"time.hours_minutes_ago",{h:o,m:String(s).padStart(2,"0")})}render(){const t=this._getSnapshotUrl(),e=this._getVideoUrl(),i=this._entity()?.last_changed;return W`
      <div
        class="camera-panel ${t?"camera-panel--clickable":""}"
        @click=${()=>{t&&(this._showModal=!0)}}
      >
        ${t?W`
              <img
                src="${t}"
                alt="${vt(this.hass,"camera.title")}"
                class="camera-img"
              />
              <div class="camera-overlay">
                <ha-icon
                  icon="${e?"mdi:play-circle-outline":"mdi:image-outline"}"
                ></ha-icon>
                ${i?W`<span class="camera-ts"
                      >${this._relativeTime(i)}</span
                    >`:Y}
              </div>
            `:W`
              <div class="camera-placeholder">
                <ha-icon icon="mdi:paw"></ha-icon>
                <span>${vt(this.hass,"card.no_recent_activity")}</span>
              </div>
            `}
      </div>
      ...
      ${this._showModal?W`
            <div
              ...
              @click=${t=>{t.target===t.currentTarget&&(this._showModal=!1)}}
            >
              <div class="modal" role="dialog" aria-modal="true">
                <div class="modal-header">
                  <ha-icon icon="mdi:cctv"></ha-icon>
                  <span>${vt(this.hass,"camera.title")}</span>
                  <button
                    class="modal-close"
                    @click=${()=>this._showModal=!1}
                  >
                ...
                <div class="modal-body">
                  ${e?W`<video
                      ...
                      ></video>`:t?W`<img
                          ...
                          alt="${vt(this.hass,"camera.title")}"
                          class="modal-img"
                        />`:W`<p class="modal-empty">
                          ${vt(this.hass,"camera.stream_unavailable")}
                        </p>`}
                </div>
              </div>
            </div>
  ...
  `,t([ut({attribute:!1})],$t.prototype,"hass",void 0),t([ut()],$t.prototype,"entityId",void 0),t([mt()],$t.prototype,"_showModal",void 0),customElements.define("onlycat-camera-panel",$t);class xt extends lt{constructor(){super(...arguments),this.historyHours=24,this._show=!1,this._loading=!1,this._hasFetched=!1,this._error=null,this._data=[[],[],[]],this._offsetPages=0,this._zoom=null}_timeWindow(){const t=new Date;t.setHours(0,0,0,0);const e=new Date(t.getTime()-864e5*this._offsetPages),i=0===this._offsetPages?new Date:new Date(e.getTime()+864e5);return{start:e,end:i}}_isEntityOn(t){return"on"===this.hass?.states?.[t]?.state}async _load(){if(!this._loading){this._loading=!0,this._error=null;try{const{start:t,end:e}=this._timeWindow(),i=[this.eventEntityId,this.contrabandEntityId,this.humanEntityId].join(","),o=`history/period/${t.toISOString()}?filter_entity_id=${i}&end_time=${e.toISOString()}&minimal_response&no_attributes&significant_changes_only=false`,s=await this.hass.callApi("GET",o);if(!Array.isArray(s))return void(this._data=[[],[],[]]);const r=[this.eventEntityId,this.contrabandEntityId,this.humanEntityId],n=e.getTime()-t.getTime(),a=[[],[],[]];console.debug("[OnlyCat] raw API response:",s.map(t=>`${t[0]?.entity_id}: ${t.length} entries`));for(const i of s){if(!i?.length)continue;const o=i[0],s=r.indexOf(o.entity_id);if(-1===s)continue;let c=null,l=0;for(const e of i){let i,o;const r=e;if("state"in r)i=r.state,o=new Date(r.last_changed).getTime();else{i=r.s;const t=r.lc??r.lu;if(void 0===t)continue;o=t>1e12?t:1e3*t}"on"===i&&null===c?(c=o,l++):"on"!==i&&null!==c&&(a[s].push({start:Math.max(0,(c-t.getTime())/n),end:Math.min(1,(o-t.getTime())/n),startTs:c,endTs:o}),c=null)}null!==c&&a[s].push({start:Math.max(0,(c-t.getTime())/n),end:1,startTs:c,endTs:e.getTime()}),console.debug(`[OnlyCat] ${o.entity_id}: ${i.length} entries → ${l} "on" transitions → ${a[s].length} periods`)}for(let t=0;t<a.length;t++){const e=["event","contraband","human"][t];for(const i of a[t])console.debug(`[OnlyCat] ${e} period: ${new Date(i.startTs).toISOString()} → ${new Date(i.endTs).toISOString()} (${Math.round((i.endTs-i.startTs)/1e3)}s)`)}console.debug("[OnlyCat] history parsed",a.map((t,e)=>`${["event","contraband","human"][e]}:${t.length}`)),this._data=a,this._hasFetched=!0}catch(t){console.error("[OnlyCat] history error",t),this._error=vt(this.hass,"history.error")}finally{this._loading=!1}}}_toggle(){this._show=!this._show,this._show&&this._load()}_navPrev(){this._offsetPages++,this._load()}_navNext(){this._offsetPages>0&&(this._offsetPages--,this._load())}_formatDateRange(){const{start:t}=this._timeWindow(),e=this.hass?.locale?.language??"en";return new Intl.DateTimeFormat(e,{weekday:"short",month:"short",day:"numeric"}).format(t)}_axisLabels(){const{start:t,end:e}=this._timeWindow(),i=e.getTime()-t.getTime(),o=t=>{const e=t.getHours(),i=t.getMinutes();return 0===i?`${e}h`:`${e}h${String(i).padStart(2,"0")}`},s=[];for(let r=0;r<=24;r+=6){const n=t.getTime()+36e5*r;if(n>e.getTime()+1)break;const a=Math.min(1,(n-t.getTime())/i);s.push({label:0===r?"0h":o(new Date(n)),frac:a})}if(0===this._offsetPages){(s[s.length-1]?.frac??0)<.97&&s.push({label:o(e),frac:1})}return s}_formatTooltip(t,e){const i=this.hass?.locale?.language??"en",o=t=>t.toLocaleTimeString(i,{hour:"2-digit",minute:"2-digit"}),s=Math.round((e-t)/1e3),r=s<60?`${s}s`:s<3600?`${Math.floor(s/60)}min${s%60>0?" "+s%60+"s":""}`:`${Math.floor(s/3600)}h ${Math.floor(s%3600/60)}min`;return`${o(new Date(t))} – ${o(new Date(e))} (${r})`}_onBarEnter(t,e,i,o){clearTimeout(this._zoomTimer);const s=this._data[o]?.indexOf(t)??0;this._zoom={centerTs:(t.startTs+t.endTs)/2,highlightStartTs:t.startTs,highlightEndTs:t.endTs,color:e,label:i,rowIndex:o,eventIndex:s}}_zoomNavigate(t){if(!this._zoom)return;const e=this._data[this._zoom.rowIndex],i=this._zoom.eventIndex+t;if(i<0||i>=e.length)return;const o=e[i];this._zoom={...this._zoom,eventIndex:i,centerTs:(o.startTs+o.endTs)/2,highlightStartTs:o.startTs,highlightEndTs:o.endTs}}_onBarLeave(){clearTimeout(this._zoomTimer),this._zoomTimer=setTimeout(()=>{this._zoom=null},200)}_renderZoom(){const t=this._zoom,e=t.highlightEndTs-t.highlightStartTs,i=Math.max(18e5,Math.min(72e5,30*e)),o=t.centerTs-i/2,s=t.centerTs+i/2,r=i,n=this.hass?.locale?.language??"en",a=t=>{const e=new Date(t),i=e.getMinutes();return`${e.getHours()}h${i>0?String(i).padStart(2,"0"):""}`},c=Math.round((t.highlightEndTs-t.highlightStartTs)/1e3),l=c<60?`${c}s`:`${Math.floor(c/60)}min${c%60?" "+c%60+"s":""}`,d=(this._data[t.rowIndex]??[]).filter(t=>t.endTs>=o&&t.startTs<=s);return W`
      <div
        class="zoom-overlay"
      ...
      >
        <div class="zoom-header-info">
          <span class="zoom-time">${h=t.highlightStartTs,new Date(h).toLocaleTimeString(n,{hour:"2-digit",minute:"2-digit",second:"2-digit"})}</span>
        ...
        <div class="zoom-track">
          <svg class="zoom-svg" viewBox="0 0 600 28" preserveAspectRatio="none">
            ${d.map(e=>{const i=Math.max(0,(e.startTs-o)/r*600),s=Math.min(600,(e.endTs-o)/r*600),n=Math.max(4,s-i),a=e.startTs===t.highlightStartTs&&e.endTs===t.highlightEndTs;return V`<g>
    ...
    `;var h}_renderChart(){const t=[{label:vt(this.hass,"history.row_flap"),color:"var(--history-flap-color, #29b6f6)",events:this._data[0]??[]},{label:vt(this.hass,"history.row_prey"),color:"var(--history-contraband-color, #e53935)",events:this._data[1]??[]},{label:vt(this.hass,"history.row_human"),color:"var(--history-human-color, #ab47bc)",events:this._data[2]??[]}];return W`
      ...
      <div class="event-section">
        ...
        ${this._show?this._loading&&!this._hasFetched?W`<div class="history-status">
                <ha-circular-progress
                  ...
                  size="small"
                ></ha-circular-progress>
                <span>${vt(this.hass,"history.loading")}</span>
  ...
  `,t([ut({attribute:!1})],xt.prototype,"hass",void 0),t([ut()],xt.prototype,"eventEntityId",void 0),t([ut()],xt.prototype,"contrabandEntityId",void 0),t([ut()],xt.prototype,"humanEntityId",void 0),t([ut({type:Number})],xt.prototype,"historyHours",void 0),t([mt()],xt.prototype,"_show",void 0),t([mt()],xt.prototype,"_loading",void 0),t([mt()],xt.prototype,"_hasFetched",void 0),t([mt()],xt.prototype,"_error",void 0),t([mt()],xt.prototype,"_data",void 0),t([mt()],xt.prototype,"_offsetPages",void 0),t([mt()],xt.prototype,"_zoom",void 0),customElements.define("onlycat-activity-history",xt);class wt extends lt{constructor(){super(...arguments),this._showRebootConfirm=!1}get _deviceId(){return this._config?.device_id??""}get _cameraEntityId(){return`camera.${this._deviceId}_last_activity_video`}get _lockEntityId(){return`binary_sensor.${this._deviceId}_lock`}get _connectivityEntityId(){return`binary_sensor.${this._deviceId}_connectivity`}get _policyEntityId(){return`select.${this._deviceId}_policy`}get _unlockEntityId(){return`button.${this._deviceId}_unlock`}get _rebootEntityId(){return`button.${this._deviceId}_reboot`}get _eventEntityId(){return`binary_sensor.${this._deviceId}_event`}get _contrabandEntityId(){return`binary_sensor.${this._deviceId}_contraband`}get _humanEntityId(){return`binary_sensor.${this._deviceId}_human`}static getStubConfig(){return{name:"",device_id:"",show_title:!0}}static getConfigElement(){return document.createElement("onlycat-home-assistant-card-editor")}setConfig(t){if(!t)throw new Error("Invalid configuration.");this._config={name:t.name??"",device_id:t.device_id??"",show_title:!1!==t.show_title}}getCardSize(){return 5}_entity(t){return this.hass?.states?.[t]}_isOn(t){return"on"===this._entity(t)?.state}_t(t){return vt(this.hass,t)}_tf(t,e){return yt(this.hass,t,e)}_onUnlock(){this._entity(this._unlockEntityId)&&this.hass.callService("button","press",{entity_id:this._unlockEntityId})}_onRebootConfirm(){this._entity(this._rebootEntityId)&&(this.hass.callService("button","press",{entity_id:this._rebootEntityId}),this._showRebootConfirm=!1)}_onPolicyChange(t){const e=t.target.value;e&&this.hass.callService("select","select_option",{entity_id:this._policyEntityId,option:e})}_renderStatusPills(){const t=this._isOn(this._connectivityEntityId),e=this._isOn(this._lockEntityId);return W`
      <div class="status-pills">
        <div class="pill ${e?"pill--locked":"pill--unlocked"}">
          ...
          ></ha-icon>
          <span
            >${vt(this.hass,e?"card.locked":"card.unlocked")}</span
          >
        </div>
        <div class="pill ${t?"pill--online":"pill--offline"}">
          <ha-icon icon="${t?"mdi:wifi":"mdi:wifi-off"}"></ha-icon>
          <span
            >${vt(this.hass,t?"card.connected":"card.offline")}</span
          >
        </div>
      </div>
    `}_renderPolicy(){const t=this._entity(this._policyEntityId),e=t?.attributes?.options??[],i=t?.state??"";return W`
      <div class="row-section">
        ...
        <span class="section-label">${vt(this.hass,"card.policy")}</span>
        ${t?W`
              <select
                class="policy-select"
                .value=${i}
            ...
            `:W`<span class="unavailable"
              >${vt(this.hass,"card.unavailable")}</span
            >`}
      </div>
    `}_renderActions(){return W`
      <div class="actions-row">
        <button
          ...
          title="${vt(this.hass,"actions.unlock_title")}"
        >
          <ha-icon icon="mdi:lock-open-variant"></ha-icon>
          <span>${vt(this.hass,"actions.unlock")}</span>
        </button>

        <button
          class="action-btn action-btn--secondary"
          @click=${()=>this._showRebootConfirm=!0}
          title="${vt(this.hass,"actions.restart_title")}"
        >
          <ha-icon icon="mdi:restart"></ha-icon>
          <span>${vt(this.hass,"actions.restart")}</span>
        </button>
      </div>
      ...
      >
        <div class="modal modal--confirm" role="dialog" aria-modal="true">
          <div class="modal-header">
            ...
            ></ha-icon>
            <span>${vt(this.hass,"confirm_restart.title")}</span>
            <button
              class="modal-close"
          ...
          <div class="modal-body">
            <p>${vt(this.hass,"confirm_restart.question")}</p>
            <p class="confirm-note">
              ${vt(this.hass,"confirm_restart.note")}
            </p>
          </div>
          <div class="modal-footer">
            ...
            >
              <ha-icon icon="mdi:restart"></ha-icon>
              ${vt(this.hass,"actions.restart")}
            </button>
          </div>
        </div>
      </div>
    `:Y}render(){if(!this.hass||!this._config)return Y;const t=this._config.name||vt(this.hass,"card.name_default");return this._config.device_id?W`
      <ha-card>
    ...
    `:W`
        <ha-card>
          ...
          >
            ...
            ></ha-icon>
            ${vt(this.hass,"card.config_required")}
          </div>
        </ha-card>
      `}}wt.styles=n`
    :host {
