...
from homeassistant.components.camera import Camera, CameraEntityFeature, PLATFORM_SCHEMA
from homeassistant.const import CONF_FILE_PATH, CONF_NAME
import homeassistant.helpers.config_validation as cv

VIDEO_URL = "/local/onlycat_card/video_test.mp4"
POSTER_URL = "/local/onlycat_card/video_poster.jpg"
# HA's stream integration fetches this via ffmpeg – served without auth from /local/
VIDEO_STREAM_URL = "http://localhost:8123/local/onlycat_card/video_test.mp4"
...
class StaticFileCamera(Camera):
    """Camera entity that serves a static image file and exposes a test video stream."""

    # Declare STREAM support so HA's more-info dialog offers video playback.
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Expose video_url and poster_url attributes."""
    ...
    async def stream_source(self) -> str | None:
        """Return the stream source URL (served as HLS via ffmpeg by HA's stream integration)."""
